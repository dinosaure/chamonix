...
type 'e t' =
  ...
  | Val : int -> 'e t'
  | Len : [ `Byte | `UTF_8 ] -> 'e t'
  | Len_of : [ `Byte | `UTF_8 ] * ('e, string ref) Var.t -> 'e t'
  | Var : ('e, int ref) Var.t -> 'e t'
...
let rec eval :
 ...
 fun ~gamma ~mode ~state -> function
  ...
  | Limit -> State.limit ~mode state
  | Val v -> v
  | Len e -> State.len ~encoding:e state
  | Len_of (`Byte, var) -> (
  ...
  | Var var -> (
      ...
      | Ty.Value (Int, n) -> !n
      | _ -> safe ())
  | Bin (`Add, a, b) -> eval ~gamma ~mode ~state a + eval ~gamma ~mode ~state b
  | Bin (`Sub, a, b) -> eval ~gamma ~mode ~state a - eval ~gamma ~mode ~state b
  | Bin (`Mul, a, b) -> eval ~gamma ~mode ~state a * eval ~gamma ~mode ~state b
  | Bin (`Div, a, b) -> eval ~gamma ~mode ~state a / eval ~gamma ~mode ~state b
...
let typ ~constants ~gamma expr =
  let rec go :
   ...
   fun ~gamma expr ->
    ...
    | (Size_of (`Name name) | Len_of (`Name name)) as len_of -> (
        ...
        try
          ...
          | Var.V (_, _, t') ->
              Error
                (Error.v expr gamma
                   [ Error.Type_mismatch { a = Ty.V Ty.String; b = Ty.V t' } ])
        with Var.Unbound_variable _ ->
    ...
    | Name name -> (
        try
          ...
          | Var.V (_, _, t') ->
              Error
                (Error.v expr gamma
                   [ Error.Type_mismatch { a = Ty.V Ty.Int; b = Ty.V t' } ])
        with Var.Unbound_variable _ ->
          Error (Error.v expr gamma [ Error.Unbound_variable ]))
    ...
    | (Plus (a, b) | Minus (a, b) | Multiply (a, b) | Divide (a, b)) as
      operation -> (
        let operator =
...
let rec pp ~gamma : bound_variables Fmt.t =
 fun ppf -> function
  ...
  | Size_of (`Literal_string v) ->
      Fmt.pf ppf "sizeof %a" (Literal_string.pp ~gamma) v
  | Len -> Fmt.string ppf "len"
  | Len_of (`Name v) -> Fmt.pf ppf "lenof %s" v
  | Len_of (`Literal_string v) ->
      Fmt.pf ppf "lenof %a" (Literal_string.pp ~gamma) v
