...
let split ~escape str =
  ...
  let extract idx =
    let len = ref 0 in
    while str.[idx + !len] <> snd escape
    do incr len done ;
    idx + !len + 1, `Expand (String.sub str idx !len) in
...
let rec to_utf_8_string ~gamma ~escape str =
  ...
  let map = function
    ...
    | `Expand "'" -> `String "'"
    | `Expand str ->
      ( try let cp = Unicode.unicode (Lexing.from_string str) in
  ...
  | lst, [] -> String.concat "" (List.map (function `String v -> v | _ -> safe ()) lst)
  | _, rest ->
   let pp ppf = function
     | `Expand str -> Fmt.pf ppf "(@[<1>`Expand@ %S@])" str
     | `Not_found str -> Fmt.pf ppf "(@[<1>`Not_found %S@])" str
     | `String _ -> safe () in
   Fmt.failwith "The string %S can not be compiled! Errors with: %a."
     str Fmt.(Dump.list pp) rest

let to_utf_8_string ~gamma { escape; contents; } =
  to_utf_8_string ~gamma ~escape contents

let pp ~gamma ppf v =
  Fmt.string ppf (to_utf_8_string ~gamma v)
